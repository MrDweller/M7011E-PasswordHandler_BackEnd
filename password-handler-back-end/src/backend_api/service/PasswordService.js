...
exports.regeneratePassword = function (body, uname, userToken) {
  return new Promise(function (resolve, reject) {
    let masterpwd = body["password"];
    let website_url = body["website_url"];
    let website_uname = body["website_uname"];
    backEndHandler.regeneratePassword(masterpwd, website_url, website_uname, uname, userToken, (result) => {
      if (result instanceof ServerErrors.InvalidToken) {
        reject(403);
        return;
      }
      ...
      if (result instanceof ServerErrors.NoPasswordFound) {
        reject(404);
        return;
      }
      if (result instanceof ServerErrors.InternalServerError) {
...
exports.deletePassword = function (body, uname, userToken) {
  return new Promise(function (resolve, reject) {
    let masterpwd = body["password"];
    let website_url = body["website_url"];
    let website_uname = body["website_uname"];
    backEndHandler.deletePassword(masterpwd, website_url, website_uname, uname, userToken, (result) => {
      if (result instanceof ServerErrors.InvalidToken) {
        reject(403);
        return;
      }
